...
const UserItems = () => {
    ...
    async (pg = 1, append = false) => {
      try {
        ...
        if (res.data?.success) {
          ...
          setUserItems((prev) => (append ? [...prev, ...items] : items));
        } else {
  ...
  const confirmDelete = async () => {
    ...
    setUserItems((prev) => prev.filter((it) => it._id !== itemId));
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const remainingItems = userItems.length - 1;
              ...
              <Col key={item._id} xs={12} md={6} lg={4} xl={3}>
                <Card className="h-100 shadow-sm hover-lift transition-all">
                  ...
                  <Card.Body className="d-flex flex-column">
                    ...
                    {/* Item Details - FIXED: Changed Card.Text to div */}
